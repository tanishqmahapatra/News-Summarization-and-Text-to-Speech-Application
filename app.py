...
def analyze_news(company_name):
    ...
    # Generate Hindi Audio Summary using deep_translator instead of googletrans
    translator = GoogleTranslator(source='auto', target='hi')
    hindi_summary = translator.translate(final_sentiment)
    tts = gTTS(text=hindi_summary, lang='hi')
    audio_filename = "/home/user/app/hindi_summary.mp3"
    tts.save(audio_filename)
    report["Audio"] = "[Play Hindi Speech]"

    return json.dumps(report, indent=4), os.path.abspath(audio_filename)

def play_audio(company_name):
    _, audio_file = analyze_news(company_name)
    return audio_file
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        output_text = gr.Textbox(label="Analysis Report", interactive=False)
        audio_output = gr.Audio(label="Hindi Summary Audio")

    submit_button.click(fn=analyze_news, inputs=company_input, outputs=[output_text, audio_output])
